...
static mgt::Status ResidualFc1Impl(
    const CudaMlpShape&s,const float*w,const float*aff,std::uint32_t b,
    std::uint32_t lr,std::uint32_t gr,float*fw,const mgt::BatchNormTrainingPlan&p,
    float*d_fc1,float*weight_grad,float*affine_grad,float*residual_grad,
    float*input_grad,NcclRankContext*ctx,cublasHandle_t blas,cudaStream_t st,
    bool fuse_residual) {
    const std::uint64_t bh2=static_cast<std::uint64_t>(lr)*s.hd2;
    float*block_inputs=fw+static_cast<std::uint64_t>(lr)*s.hd1;
    float*fc1_activations=block_inputs+static_cast<std::uint64_t>(s.residual_blocks+1U)*bh2;
    float*bn=fc1_activations+static_cast<std::uint64_t>(s.residual_blocks)*bh2;
    float*activated=fc1_activations+static_cast<std::uint64_t>(b)*bh2;
    float*input=block_inputs+static_cast<std::uint64_t>(b)*bh2;
    const auto&q=p.sites[2+2*b];
    if(ReluBnBackward<true>(activated,d_fc1,lr,gr,q.logical_features,q.physical_stride,
        aff+q.affine_offset,bn+q.inv_std_offset,bn+q.normalized_offset,d_fc1,
        affine_grad+q.affine_offset,affine_grad+p.logical_feature_count+q.affine_offset,
        bn+p.reduction_offset,ctx,st)!=mgt::Status::kOk)return mgt::Status::kCudaFailure;
    float alpha=1,weight_beta=0,input_beta=fuse_residual?1.0f:0.0f;
    float*destination=fuse_residual?residual_grad:input_grad;
    if(cublasSgemm(blas,CUBLAS_OP_N,CUBLAS_OP_T,s.hd2,s.hd2,lr,&alpha,d_fc1,
        s.hd2,input,s.hd2,&weight_beta,weight_grad+F1W(s,b),s.hd2)!=CUBLAS_STATUS_SUCCESS||
       cublasSgemm(blas,CUBLAS_OP_T,CUBLAS_OP_N,s.hd2,lr,s.hd2,&alpha,
        w+F1W(s,b),s.hd2,d_fc1,s.hd2,&input_beta,destination,s.hd2)!=CUBLAS_STATUS_SUCCESS)
        return mgt::Status::kCudaFailure;
    ColumnSum<4><<<(s.hd2+3U)/4U,T,0,st>>>(d_fc1,lr,q.logical_features,s.hd2,
        weight_grad+F1B(s,b));
    if(!fuse_residual)
        AddInPlace<<<static_cast<unsigned>((bh2+T-1)/T),T,0,st>>>(input_grad,residual_grad,bh2);
    return launch()?mgt::Status::kOk:mgt::Status::kCudaFailure;
}
mgt::Status LaunchMlpBatchNormResidualStackBackward(
    const CudaMlpShape&s,const float*w,const float*aff,std::uint32_t lr,
    std::uint32_t gr,float*fw,const mgt::BatchNormTrainingPlan&p,float*block_grad,
    float*weight_grad,float*affine_grad,float*d_fc1,float*d_residual,
    NcclRankContext*ctx,cublasHandle_t blas,cudaStream_t st) {
    if(ValidateCudaMlpShape(s)!=mgt::Status::kOk||!w||!aff||!lr||gr<lr||!fw||
       p.sites.size()!=2+2*s.residual_blocks||!block_grad||!weight_grad||
       !affine_grad||!d_fc1||!d_residual||!ctx||!blas)
        return mgt::Status::kInvalidConfig;
    float*current_grad=block_grad;
    float*residual_buffer=d_residual;
    for(std::uint32_t b=s.residual_blocks;b-->0;){
        auto z=ResidualFc2Impl(s,w,aff,b,lr,gr,fw,p,current_grad,weight_grad,
            affine_grad,d_fc1,residual_buffer,ctx,blas,st,false);
        if(z!=mgt::Status::kOk)return z;
        // beta=1 folds the skip gradient into dX. Ping-pong keeps both operands
        // distinct. One odd-depth tail uses the legacy add to preserve the API's
        // fixed block_grad output pointer without an extra copy.
        const bool legacy_tail=(s.residual_blocks&1U)&&b==0;
        z=ResidualFc1Impl(s,w,aff,b,lr,gr,fw,p,d_fc1,weight_grad,affine_grad,
            residual_buffer,current_grad,ctx,blas,st,!legacy_tail);
        if(z!=mgt::Status::kOk)return z;
        if(!legacy_tail)std::swap(current_grad,residual_buffer);
    }
    return NcclAllreduceSumFloat(weight_grad+R0(s),OW(s)-R0(s),ctx,st);
}
