...
mgt::Status LaunchStridedSyncBatchNormForwardFused(const float* x, int local_rows, int global_rows, int cols, int row_stride, const float* gamma, const float* beta, float* running_mean, float* running_var, float momentum, float epsilon, float* y, float* mean, float* inv_std, float* normalized, float* stats_workspace, NcclRankContext* context, cudaStream_t stream);
mgt::Status LaunchStridedSyncBatchNormBackwardFused(const float* dy, int local_rows, int global_rows, int cols, int row_stride, const float* gamma, const float* inv_std, const float* normalized, float* dx, float* dgamma, float* dbeta, float* stats_workspace, NcclRankContext* context, cudaStream_t stream);
}