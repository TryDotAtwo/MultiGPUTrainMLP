...
void CpuForwardOne(const mgt_cuda::CudaMlpShape& s, const std::vector<float>& w, const mgt::TrainStateStorage& state, float* output) {
    ...
    const std::uint64_t output_bias = output_weight + static_cast<std::uint64_t>(s.hd2) * s.output_dim;
    ...
    for (std::uint32_t out = 0; out < s.output_dim; ++out) {
        float y = w[output_bias + out];
        for (std::uint32_t j = 0; j < s.hd2; ++j) y += cur[j] * w[output_weight + static_cast<std::uint64_t>(j) * s.output_dim + out];
        output[out] = y;
    }
}
}  // namespace
...
int main() {
    ...
    const mgt_cuda::CudaMlpShape shape{4, 8, 5, 3, 1, 3};
    ...
    std::vector<float> cpu(static_cast<std::uint64_t>(kSamples) * shape.output_dim, 0.0f);
    for (std::uint32_t sample = 0; sample < kSamples; ++sample) CpuForwardOne(shape, weights, states[sample], cpu.data() + static_cast<std::uint64_t>(sample) * shape.output_dim);
    float* d_weights = nullptr; mgt::TrainStateStorage* d_states = nullptr; float* d_outputs = nullptr;
    ...
    if (Check(cudaMalloc(&d_outputs, cpu.size() * sizeof(float))) != 0) return EXIT_FAILURE;
    if (Check(cudaMemcpy(d_weights, weights.data(), params * sizeof(float), cudaMemcpyHostToDevice)) != 0) return EXIT_FAILURE;
    if (Check(cudaMemcpy(d_states, states.data(), kSamples * sizeof(mgt::TrainStateStorage), cudaMemcpyHostToDevice)) != 0) return EXIT_FAILURE;
    ...
    std::vector<float> gpu(cpu.size(), 0.0f);
    if (Check(cudaMemcpy(gpu.data(), d_outputs, gpu.size() * sizeof(float), cudaMemcpyDeviceToHost)) != 0) return EXIT_FAILURE;
    for (std::uint64_t item = 0; item < gpu.size(); ++item) if (std::fabs(gpu[item] - cpu[item]) > 1.0e-5f) { std::fprintf(stderr, "forward mismatch item=%llu gpu=%0.9g cpu=%0.9g\n", static_cast<unsigned long long>(item), gpu[item], cpu[item]); return EXIT_FAILURE; }
